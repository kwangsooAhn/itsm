...
(function(global, factory) {
    ...
    let targetArray = []; // 사용자 선택 modal 내 선택된 사용자 데이터
    let userInfo = null;
    ...
    function Dropdown(property) {
        ...
        this.template =
            ...
            `<button id="${objectId}_add" type="button" class="btn__ic theme--extra">` +
    ...
    function UserSearch(property) {
        ...
        const targetOptions = [
            ...
            return `<option value='${option.value}' ` +
                `${property.targetCriteria === option.value ? 'selected=\'true\'' : ''}>` +
                `${aliceJs.filterXSS(option.text)}</option>`;
        }).join('');
        this.template = `${requiredTemplate}` +
            `<div class="flex-row mt-2">
    ...
    function changeTargetCriteriaHandler(e) {
        ...
        if (e.target.value === 'organization') {
            ...
            const customTemplate = `<div class="align-right">
                <button type="button" class="btn__text--box theme--secondary" id="searchUserList">`   +
    ...
    function openUserListModal(e) {
        ...
        // targetArray = isMulti ? targetUserArray : defaultCustomUser;
     ...
     * 사용자 조회
     */
    function getTargetUserList(search, isMulti, userArray, showProgressbar) {
    ...
    function addUserInTargetUser(dataList) {
        ...
        dataList.forEach( (data) => {
            listTemplate += `<div class="flex-row mt-2 user-search-item">` +
                ...
                `</div>` +
                `<div class="flex-column">` +
                    `<button type="button" class="item-remove theme--extra ml-2 mr-2 mt-2 mb-2 user-search-delete-btn"` +
    ...
    function addCIAttribute(type, data, parent) {
        ...
        switch (type) {
            ...
            case 'custom-code': {
                ...
                customBtnElem.className = 'btn__ic theme--secondary';
            ...
            case 'group-list': {
                ...
                if (typeof attributeValue.option !== 'undefined' && data.childAttributes.length > 0) {
                    ...
                    if (displayMode === 'edit') {
                        ...
                        addBtn.type = 'button';
                        addBtn.className = 'btn__ic theme--secondary button-attribute-add';
