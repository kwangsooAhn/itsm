...
function isValidPassword(elementId) {
    ...
    if (blankReg.test(password)) {
        aliceJs.alertWarning(i18n.msg('validation.msg.pwNotContainSpace'));
        return false;
    }
    ...
    if (lowerCaseReg.test(password) || upperCaseReg.test(password) || specialCharReg.test(password) || numberReg.test(password)) {
        if (password.length < 10 || password.length > 20) {
            aliceJs.alertWarning(i18n.msg('validation.msg.pwOneBetween10And20'));
            return false;
        }
    }
    ...
    if ((searchUpperCase < 0 && searchLowerCase < 0) || (searchLowerCase < 0 && searchSpecialChar < 0) ||
        ...
        if (password.length < 8 || password.length > 20) {
            aliceJs.alertWarning(i18n.msg('validation.msg.pwTwoBetween8And20'));
            return false;
        }
    }
    ...
    if (password.search(userId) > -1) {
        aliceJs.alertWarning(i18n.msg('validation.msg.pwContainsId'));
        return false;
    }
    ...
    if (password.search(emailId[0]) > -1) {
        aliceJs.alertWarning(i18n.msg('validation.msg.pwContainsEmail'));
        return false;
    }
...
function isValidEmail(elementId) {
    ...
    if (!emailReg.test(email)) {
        aliceJs.alertWarning(i18n.msg('validation.msg.checkEmailFormat'));
        return false;
    }
    return true;
}
...
function isValidUserId(elementId) {
    ...
    if (!idReg.test(userId)) {
        aliceJs.alertWarning(i18n.msg('validation.msg.checkUserIdFormat'));
        return false;
    }
    return true;
}
...
function isValidRgb(elementId, callbackFunc) {
    ...
    if (!rgbReg.test(rgb)) {
        aliceJs.alertWarning(i18n.msg('validation.msg.checkRgbFormat'), callbackFunc);
        return false;
    }
    return true;
}
