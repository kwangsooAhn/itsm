...
interface WfElementRepository: JpaRepository<WfElementEntity, String> {
    ...
    @Query("SELECT elem FROM WfElementEntity elem " +
            "WHERE elem.processId = :processId " +
            "AND elem.elementType = :elementType")
    fun findUserTaskByProcessId(processId : String,
                                elementType: String = WfElementConstants.ElementType.USER_TASK.value) : MutableList<WfElementEntity>
