 ...
 */
class WfProcessSimulationArrow(repository: WfElementRepository) : WfProcessSimulationElement(repository) {
    ...
    override fun validate(element: WfElementEntity): Boolean {
        ...
        val sourceElement = repository.getOne(sourceElementId!!)
        if ((sourceElement.elementType == WfElementConstants.ElementType.EXCLUSIVE_GATEWAY.value
                    || sourceElement.elementType == WfElementConstants.ElementType.EXCLUSIVE_GATEWAY.value
                    || sourceElement.elementType == WfElementConstants.ElementType.EXCLUSIVE_GATEWAY.value)
            && emptyCondition
        ) {
            return false
    ...
    override fun failInfo(): String {
        return "Arrow connector simulation failed"
    }
}
