...
class FolderService(private val restTemplate: RestTemplateProvider) {
    ...
    fun getInstance(tokenId: String?, searchValue: String?): List<RestTemplateRelatedInstanceDto>? {
        var relatedInstance: MutableList<RestTemplateRelatedInstanceDto>? = null
        val params = LinkedMultiValueMap<String, String>()
        params["tokenId"] = tokenId
        params["searchValue"] = searchValue

        val urlDto = when (tokenId) {
            null -> {
                RestTemplateUrlDto(callUrl = RestTemplateConstants.Instance.GET_ALL_INSTANCE.url, parameters = params)
            }
            else -> {
