...
public interface NoticeRepository : JpaRepository<NoticeEntity, String> {
	...
	fun findAllByTitle(keyword: String, fromDate: LocalDateTime, toDate: LocalDateTime): MutableList<NoticeEntity>

	@Query("select a from NoticeEntity a where a.createUserid like %?1% and a.createDt between ?2 and ?3" )
	fun findAllByWriter(keyword: String, fromDate: LocalDateTime, toDate: LocalDateTime): MutableList<NoticeEntity>

	@Query("select a from NoticeEntity a where ( a.noticeTitle like %?1% or a.createUserid like %?2% ) and a.createDt between ?3 and ?4")
	fun findAllCheck(keyword: String, copyKeyword: String, fromDate: LocalDateTime, toDate: LocalDateTime): MutableList<NoticeEntity>
}